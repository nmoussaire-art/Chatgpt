 ...
 * baseline. Cards whose behaviour has never been observed on this phone say so on their face
 * rather than quietly substituting a plausible-looking number.
 */
@Composable
    ...
    LazyColumn(
        ...
        when (val current = state) {
            is ForecastUiState.Ready -> {
                ...
                if (loading) {
                    item { LinearProgressIndicator(modifier = Modifier.fillMaxSize()) }
                }
...
private fun ScenarioCard(outcome: ScenarioOutcome, baseline: ScenarioOutcome?) {
    ...
    BatteryCastCard {
        SectionHeader(title = outcome.scenario.displayName, subtitle = outcome.scenario.description)
