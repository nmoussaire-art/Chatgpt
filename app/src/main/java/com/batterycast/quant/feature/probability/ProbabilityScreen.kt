...
/**
 * Probability of lasting, hour by hour, and how it shifts under different usage.
 *
 * Every bar is a count over simulated paths, not a curve fitted to a point estimate — so a bar at
 * 84 % means 84 % of the simulated futures were still above the reserve at that time.
 */
@Composable
                ...
                item {
                    BatteryCastCard {
                        ...
                        val point = current.forecast.curve.minByOrNull {
                            kotlin.math.abs(it.atMs - current.target.atMs)
                        }
                        if (point == null) {
                            Text(
                ...
                item {
                    BatteryCastCard {
                        ...
                        scenarios.forEach { outcome ->
                            ...
                            if (probability == null) {
                                Text(
                                    text = "${outcome.scenario.displayName}: ${outcome.confidence.label.lowercase()}",
                                    style = MaterialTheme.typography.bodyMedium,
                                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                                )
                            } else {
 ...
 */
private fun probabilityAbove(
    point: com.batterycast.quant.forecasting.sim.ForecastPoint,
    level: Double,
): Double {
